...
<?php
    include 'koneksi.php';
    $get = $_GET['ubah'];
    $query = $koneksi->query("SELECT * FROM tbl_petugas WHERE id_petugas = '$get'");
    $row = mysqli_fetch_array($query);
?>
<body style="width:100%;height:100vh;display: flex;flex-direction: column;align-items: center;justify-content:center;">

<form method="post" action="../admin/edit.php?ubahp=<?= $row['id_petugas'] ?>" class="modal-box">
                
                <label class="form-control w-full max-w-xs">
                    <div class="label">
                        <span class="label-text">Nama Lengkap</span>
                    </div>
                    <input type="text" name="nama" class="input input-bordered w-full max-w-xl" value="<?= $row['nama_petugas'] ?>" />
                </label>
                <label class="form-control w-full max-w-xs">
                    <div class="label">
